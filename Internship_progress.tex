...
\setlength{\textwidth}{17cm}     % Largeur de la zone de texte (par défaut ~12cm)
\setlength{\textheight}{24cm}    % Hauteur de la zone de texte (par défaut ~19cm)
\setlength{\oddsidemargin}{-0.5cm} % Marge gauche pour les pages impaires
\setlength{\evensidemargin}{-0.5cm}% Marge gauche pour les pages paires
\setlength{\topmargin}{-1.5cm}     % Marge en haut

\graphicspath{
    {images_lateX/TDA/}
    {images_lateX/phase_final_repulsive/}
    {images_lateX/phase_final_adhesive/}
    {images_lateX/test_distance/}
}
...
Following method is explained in two pages in the section 3 of \cite{sathiyakumar2024detecting}
...
Using theoretical results remained in \cite{sathiyakumar2024detecting}, $L(\epsilon)$ is proven to be a continuous function on a bounded interval $[0, \epsilon_{max}]$, belonging to a normed vector space (the $L^p$ space). This provides the mathematical justification to treat these landscapes as standard Euclidean vectors.
...
Let $\rho(x,y)$ be the local discrete density of active agents. For a given density threshold $\epsilon$, the superlevel set is the region of the domain where the density is greater than or equal to the threshold:
\begin{equation}
    \mathcal{X}_\epsilon = \{ (x,y) \in \text{Grid} \mid \rho(x,y) \geq \epsilon \}
\end{equation}
This filtration is detailed in \cite{dlotko2016rigorous} in subsection 2.2.
...
To each superlevel set we count the number of connected components as explain in the figure which come from \cite{otter2017roadmap}.

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{process_filtra_cub.png}
    \caption{barcode and landscapes}
    \label{fig:processus_tda}
\end{figure}

Computationally we will take $-\rho$ and apply functions from gudhi library where everything is implemented.
...
\subsubsection{typical patterns}
...
\textbf{Conclusion} : Finally, all distance are little between large bandes and uniform state which is not an issue because we can see on the image that the bands clearly exists but are not very distinct. Each one have it's own specifity, for the next test, huge distance between small clusters and huge clusters will be an issue but as showed the heatmap on other patterns wasserstein could be more efficient.
...
After unsuccessfully changing only the stiffness or trying to vary the parameter epsilon over the scaling assumption of macro-regime, a test that worked pretty well was to keep to ratio constant :
    ...
    \end{itemize}
We start at $\kappa$ = 1000 on a regime where agents are making bandes for one and clusters for another, then we are reducing the $\kappa$ = 10. Images of the end are in the file "test" then "keeping ratio". To describe quickly the results :
\begin{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{comparatif_metriques_amas_bandes.png}
    \caption{barcode and landscapes}
    \label{fig:comparatif}
\end{figure}

An analysis is that for $\kappa$ = 100 clusters are very small in adhesive case and huge in repulsive case (but still clusters on both sides),
and wasserstein is penalising it more than comparing trails and large clusters. for the others distances results are encouraging.\\
For bandes, $\kappa$ = 10 is uniform state for both so the three distances are describing it well. The distance deacreasing for TDA does not correspond at what a human eye will see.\\
Distances must be very high for $\kappa$ between 10 and 100 and very low between 200 and 1000, Wasserstein is the best distance to show this.

\section{Focus on fourier spectrum for pattern recognition}
\subsection{Get the good informations from the spectrum}
The strategy which was decided was to extract the informations in two steps, first keep only the relevant pixels with a well choosen cut-off to avoid the noise.
Then to make an elliptic mask of the data we kept and to get relevant quantifier from the ellips.
\subsubsection{Relevant cut-off}
We are thinking of a rationnal way to make a cut-off of the pixels in the spectrum to avoid noise whithout loosing informations. For now,
I generated 200 random configurations, took the fourier spectrum and defined the cut-off equal to the higher maximum value over all the configurations.
It seemed to work well to start having good results but needs to be justified more.
I finally took 1e6 as the cut-off.
\subsubsection{Ellips mask}
We are then calculating the semi-major axis, semi-minor axis and the angle of semi-major axis of an elliptic approximation of the datas.
To do it so we calculate order 1 moments :
$$
\bar{x} = \frac{M_{10}}{M_{00}} = \frac{\sum x \cdot I(x,y)}{\sum I(x,y)} \quad ; \quad \bar{y} = \frac{M_{01}}{M_{00}} = \frac{\sum y \cdot I(x,y)}{\sum I(x,y)}
$$
Then we use the central moments define as :
$$
\mu_{pq} = \sum_{x} \sum_{y} (x - \bar{x})^p (y - \bar{y})^q I(x, y)
$$
To calculate the covariance matrice :
$$
C = \frac{1}{M_{00}} \begin{pmatrix} \mu_{20} & \mu_{11} \\ \mu_{11} & \mu_{02} \end{pmatrix}
$$
Calling the eigenvalues $\lambda_{max}$ and $\lambda_{min}$ we get the semi-major axis $a = k \cdot \sqrt{\lambda_{max}}$
and the semi-minor axis $b = k \cdot \sqrt{\lambda_{min}}$ where k is number of std, we take 3 here to have quite all the pixels in the ellips.\\
To have the angle we get easily the eigenvectors of C which give the orientatio of both axis then get the angle by taking the arctan.\\
Finally the excentricity is defined as : $e = \sqrt{1 - \frac{\lambda_{min}}{\lambda_{max}}}$.

\subsection{Application on relevant datas}
Quick correction on section 3.2.2, after launching simulations for a longer time for bandes, instabilities appears and for all values of $\kappa$ we can observe a mix of clusters and bandes at the end of the simulation (the state is metastable),
but it is still interesting to see the result of the fourier method described before on such datas. \\
\begin{figure}[]
    \centering
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images_lateX/position_and_spectrum_ratio/montage_Ligne_Amas_k10.png}
        \caption{Amas ($\kappa$ =10)}
        \label{fig:spectre_amas_k10}
    \end{minipage}\hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images_lateX/position_and_spectrum_ratio/montage_Ligne_Bandes_k70.png}
        \caption{Bandes\\ ($\kappa$ =70)}
        \label{fig:spectre_bandes_$\kappa$=70}
    \end{minipage}\hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images_lateX/position_and_spectrum_ratio/montage_Ligne_Bandes_k800.png}
        \caption{Bandes\\ ($\kappa$ =800)}
        \label{fig:spectre_bandes_k800}
    \end{minipage}
\end{figure}
we can see that it is still working very efficiently on true simulated datas, same as for synthetical datas and an interesting point is that for mixing patterns we can see the pixels for bandes and the shape of the cluster however we loose this information with the ellips.\\
My next work is to first make sure the cluster's emergence in bandes patterns for adhesive case doesn't come from numerical instabilities. Then maybe search for it analytically by making a stability analysis starting with a bandes's repartition

\newpage

\section{Mean field limit in ECM model}
\subsection{Mean field limit of cells}
pour s'entrainer on va passer à la limite en champ moyen pour le modèle ECM

\begin{align}
d Z_k &= u_0 \alpha_k dt - \frac{1}{\zeta N} \sum_{i=1}^N \nabla_{Z_k} \phi^{FS} (X_i, \theta_i, Z_k) dt - \frac{1}{\zeta M} \sum_{l \neq k}^M \nabla_{Z_k} \phi^{SS} (Z_k - Z_l) dt, \tag{1a} \\
    d \alpha_k &= P_{\alpha_k^\perp} \circ \left[ \nu_S \bar{\alpha}_k^S dt + \nu_F \bar{\alpha}_k^F dt + \sqrt{2d_s} d \tilde{B}_t^k \right]. \tag{1b}
\end{align}

On passe à la description en mesure empirique :

$$h^M(t, x, \alpha) = \frac{1}{M} \sum_{k=1}^M \delta_{Z_k(t)}(x) \otimes \delta_{\alpha_k(t)}(\alpha)$$
$$f^N(t, x, \theta) = \frac{1}{N} \sum_{i=1}^N \delta_{X_i(t)}(x) \otimes \delta_{\theta_i(t)}(\theta)$$

Ensuite on l'évalue contre $\psi(x,\alpha)$ fonction test :  $\langle h^M, \psi \rangle = \frac{1}{M}\sum_k \psi(Z_k, \alpha_k)$\\
On applique ensuite la formule d'ito pour arriver à :
$$d \langle h^M, \psi \rangle = \frac{1}{M} \sum_{k=1}^M \left[ \nabla_x \psi(Z_k, \alpha_k) \cdot dZ_k + \nabla_\alpha \psi(Z_k, \alpha_k) \cdot d\alpha_k + d_s \Delta_\alpha \psi(Z_k, \alpha_k) dt \right]$$
Dans la partie dérivée seconde de la formule d'ito on ne gade que le terme de diffusion car les autres étant déterministe ils disparaissent dans le crochet.
On remplace ensuite d$Z_k$ et d$\alpha_k$

\begin{align*}
d \langle h^M, \psi \rangle &= \frac{1}{M} \sum_{k=1}^M \Bigg[ \nabla_x \psi(Z_k, \alpha_k) \cdot \left( u_0 \alpha_k - \frac{1}{\zeta M} \sum_{l \neq k}^M \nabla_{Z_k} \phi^{SS}(Z_k - Z_l) - \frac{1}{\zeta N} \sum_{i=1}^N \nabla_{Z_k} \phi^{FS}(X_i, \theta_i, Z_k) \right) \\
&\quad + \nabla_\alpha \psi(Z_k, \alpha_k) \cdot P_{\alpha_k^\perp} \big[ \nu_S \bar{\alpha}_k^S + \nu_F \bar{\alpha}_k^F \big] + d_s \Delta_\alpha \psi(Z_k, \alpha_k) \Bigg] dt + \nabla_\alpha \psi(Z_k, \alpha_k) \cdot \sqrt{2d_s} d\tilde{B}_t^k
\end{align*}

\subsection{Réécriture intégrale contre les mesures empiriques}
De manière plutot direct on transforme les doubles sommes en produit contre les mesures empiriques (c'est simplement une réécriture des sommes) :

\begin{align*}
d \langle h^M, \psi \rangle &= \\
\left[ \iint \nabla_x \psi(x,& \alpha) \cdot \underbrace{\left( u_0 \alpha - \frac{1}{\zeta} \iint \nabla_x \phi^{SS}(x - x') h^M(x', \alpha') dx' d\alpha' - \frac{1}{\zeta} \iint \nabla_x \phi^{FS}(x', \theta', x) f^N(x', \theta') dx' d\theta' \right)}_{\mathcal{A}[f^N, h^M](x, \alpha)} h^M(x, \alpha) dx d\alpha \right. \\
&\quad + \iint \nabla_\alpha \psi(x, \alpha) \cdot \underbrace{P_{\alpha^\perp} \left[ \nu_S \bar{\alpha}^S[h^M](x) + \nu_F \bar{\alpha}^F[f^N](x) \right]}_{\mathcal{B}[f^N, h^M](x, \alpha)} h^M(x, \alpha) dx d\alpha \\
&\quad \left. + \iint d_s \Delta_\alpha \psi(x, \alpha) h^M(x, \alpha) dx d\alpha \right] dt + \frac{\sqrt{2d_s}}{M} \sum_{k=1}^M \nabla_\alpha \psi(Z_k(t), \alpha_k(t)) \cdot d\tilde{B}_t^k
\end{align*}
avec :
$$J(x, t) = \int_{|x - y| \le r_A} \int_{\mathbb{S}^1} \alpha' h^M(t, y, \alpha') \, d\alpha' \, dy$$$$\bar{\alpha}^S[h^M](x) = \frac{J(x, t)}{|J(x, t)|}$$
Pour le terme stochastique, on calcule sa variation quadratique qui donne :
$$\frac{2d_s}{M^2} \sum_{k=1}^M \int_0^t \left| \nabla_\alpha \psi(Z_k(s), \alpha_k(s)) \right|^2 ds$$
$\psi$ étant borné, on obtient que la variation quadratique tend vers 0 quand M $\rightarrow \infty$, par doob cela donne que le terme converge fortement a fortiori faiblement vers 0.

\subsection{propagation du chaos}
Maintenant qu'on a cette formule il faut pouvoir passer à la limite faible en M et N pour obtenir une formulation faible d'une edp sur les densités h et f. Pour cela on utilise la propagation du chaos. Dans le cours de vlasov il est dit que si les termes de drift et de diffusion sont lipschitz alors il y a propagation du chaos, ainsi en supposant que les potentiels sont lipschitz on obtient un corollaire qui majore puis qui nous donne la chaoticité qui est équivalente à la convergence en loi de la mesure empirique ce qui revient donc à la convergence faible souhaitée.
Pour le voir on met le système sous la forme du cours
$$dX_t^{i,N} = \frac{1}{N}\sum_{j=1}^N \beta(X_t^{i,N}, X_t^{j,N})dt + \gamma(X_t^{i,N}) dB_t^i$$
avec $\mathbf{Y}_k(t) = \begin{pmatrix} Z_k(t) \\ \alpha_k(t) \end{pmatrix} \in \mathbb{R}^2 \times \mathbb{S}^1$
et on pose :
$$\beta(\mathbf{Y}_k, \mathbf{Y}_l) = \begin{pmatrix} - \frac{1}{\zeta} \nabla \phi^{SS}(Z_k - Z_l) \\ P_{\alpha_k^\perp} [\nu_S \alpha_l 1_{|Z_k - Z_l| \le r_A}] \end{pmatrix}$$
et :
$$b(\mathbf{Y}_k, t) = \begin{pmatrix} u_0 \alpha_k - \frac{1}{\zeta} \nabla \phi^{FS} * \rho_f(Z_k) \\ P_{\alpha_k^\perp} [\nu_F \bar{\alpha}^F[f](Z_k)] \end{pmatrix}$$
et :
$\sigma = \begin{pmatrix} 0 \\ \sqrt{2d_s} \end{pmatrix}$\\
La plupart des termes sont lipschitz cependant le terme d'alignement ne l'est pas quand J $\rightarrow$ 0, le passage n'est donc pas si clair. On l'admet pour la suite.

\subsection{Passage à la limite de champ moyen ($M, N \to \infty$)}
On suppose les convergences faibles $h^M \rightharpoonup h$ et $f^N \rightharpoonup f$.  L'équation intégrale limite s'écrit alors :

\begin{equation}
\frac{d}{dt} \iint \psi h \, dx d\alpha = \iint \Big( \nabla_x \psi \cdot \mathcal{A}[f,h] + \nabla_\alpha \psi \cdot \mathcal{B}[f,h] + d_s \Delta_\alpha \psi \Big) h \, dx d\alpha
\end{equation}

\subsection{Intégration par parties et EDP forte}
On transfère les opérateurs différentiels de la fonction test $\psi$ vers la densité $h$ en utilisant les règles usuelles d'intégration par parties ($\int \nabla \psi \cdot \vec{V} h = - \int \psi \nabla \cdot (\vec{V} h)$ et $\int \Delta \psi h = \int \psi \Delta h$). On obtient :

\begin{equation*}
\iint \psi(x, \alpha) \bigg[ \partial_t h + \nabla_x \cdot \Big( \mathcal{A}[f,h] h \Big) + \nabla_\alpha \cdot \Big( \mathcal{B}[f,h] h \Big) - d_s \Delta_\alpha h \bigg] dx d\alpha = 0
\end{equation*}

Cette égalité étant vraie pour toute fonction test $\psi$ lisse et à support compact, le crochet vaut zéro au sens des distributions, ce qui donne l'edp finale pour la densité des cellules :

\begin{equation}
\partial_t h + \nabla_x \cdot \Big( \mathcal{A}[f,h] h \Big) + \nabla_\alpha \cdot \Big( \mathcal{B}[f,h] h \Big) = d_s \Delta_\alpha h
\end{equation}

\section{Limite en champ moyen du réseau de fibres}

\subsection{Équations microscopiques et mesures empiriques}
On rappelle les EDS sur la position et l'orientation du réseau de fibres. La preuve va être foncièrement la même pour les intéractions fibres-fibres et fibres-cellules. Pour les liens on remplace la matrice de lien $p_{ij}$ par une somme sur les indices des liens directement, allant jusqu'à K (variable aléatoire). (i,j) et (j,i) sont deux liens différents donc on découpe la somme en 2 pour compter entièrement la contribution du lien réelle.

\begin{subequations}
\label{eq:micro_fibres_xi}
\begin{align}
\frac{dX_i}{dt} &= \mu_X \Bigg[ -\frac{1}{M}\sum_{k=1}^M \nabla_{X_i}\phi^{FS}(X_i, \theta_i, Z_k) - \frac{1}{N}\sum_{j=1}^N \nabla_{X_i}\phi^{FF}(X_i, \theta_i, X_j, \theta_j) \nonumber \\
&\qquad - \frac{1}{2}\sum_{\substack{k=1 \\ i(k)=i}}^K \nabla_{x_1}V(X_{i(k)}, \theta_{i(k)}, X_{j(k)}, \theta_{j(k)}) - \frac{1}{2}\sum_{\substack{k=1 \\ j(k)=i}}^K \nabla_{x_2}V(X_{i(k)}, \theta_{i(k)}, X_{j(k)}, \theta_{j(k)}) \Bigg], \tag{2a} \\
\frac{d\theta_i}{dt} &= \mu_\theta \Bigg[ -\frac{1}{M}\sum_{k=1}^M \partial_{\theta_i}\phi^{FS}(X_i, \theta_i, Z_k) - \frac{1}{N}\sum_{j=1}^N \partial_{\theta_i}\phi^{FF}(X_i, \theta_i, X_j, \theta_j) \nonumber \\
&\qquad - \frac{1}{2}\sum_{\substack{k=1 \\ i(k)=i}}^K \left(\partial_{\theta_1}V(X_{i(k)}, \dots) + \partial_{\theta_1}W^{al}(\theta_{i(k)}, \theta_{j(k)})\right) \nonumber \\
&\qquad - \frac{1}{2}\sum_{\substack{k=1 \\ j(k)=i}}^K \left(\partial_{\theta_2}V(\dots, X_{j(k)}) + \partial_{\theta_2}W^{al}(\theta_{j(k)}, \theta_{i(k)})\right) \Bigg]. \tag{2b}
\end{align}
\end{subequations}

On isole les constantes passées de l'autre côté $\mu_X = (LK_i)^{-1}$ et $\mu_\theta = \left(\xi^\perp \frac{L^3}{12}\right)^{-1}$ (il faut simplement garder en tete que $K_i$ dépend de $\theta$). On redéfinit les mesures empiriques des cellules ($h^M$), des fibres ($f^N$) et on rajoute celle du réseau ($g^K$). On symétrise $g^K$ pour ne pas avoir de problème d'ordre des liens :
\begin{align*}
f^N(t, x, \theta) &= \frac{1}{N} \sum_{i=1}^N \delta_{X_i(t)}(x) \otimes \delta_{\theta_i(t)}(\theta), \\
h^M(t, z, \alpha) &= \frac{1}{M} \sum_{k=1}^M \delta_{Z_k(t)}(z) \otimes \delta_{\alpha_k(t)}(\alpha), \\
g^K(t, x_1, \theta_1, x_2, \theta_2) &= \frac{1}{2K} \sum_{k=1}^K \Big[ \delta_{(X_{i(k)}, \theta_{i(k)})}(x_1, \theta_1) \otimes \delta_{(X_{j(k)}, \theta_{j(k)})}(x_2, \theta_2) \\
&\qquad\qquad + \, \delta_{(X_{j(k)}, \theta_{j(k)})}(x_1, \theta_1) \otimes \delta_{(X_{i(k)}, \theta_{i(k)})}(x_2, \theta_2) \Big].
\end{align*}
On définit la constante de ratio limite entre le nombre de liens et le nombre de fibres lorsqu'on passe à la limite : $\lim_{N, K \to \infty} \frac{K}{N} = \xi > 0$.

\subsection{Réécriture intégrale et symétrisation}
De la même manière on évalue la distribution des fibres contre une fonction test $\phi(x, \theta)$ :
\[
\frac{d}{dt} \langle f^N, \phi \rangle = \frac{1}{N} \sum_{i=1}^N \left[ \nabla_x \phi(X_i, \theta_i) \cdot \frac{dX_i}{dt} + \partial_\theta \phi(X_i, \theta_i) \frac{d\theta_i}{dt} \right]
\]

La seule différence sera pour la forme intégrale contre la mesure empirique des termes sur les liens. En sommant directement sur l'ensemble des liens $k = 1 \dots K$ plutôt que sur les fibres $i$, chaque lien contribue exactement deux fois (une fois pour son nœud de départ $i(k)$ et une fois pour son noeud d'arrivée $j(k)$). La somme microscopique devient directement :
\begin{align*}
-\frac{\mu_X}{2N} \sum_{k=1}^K \Big[ \nabla_{x} \phi(X_{i(k)}, \theta_{i(k)}) \cdot \nabla_{x_1} &V(X_{i(k)}, \theta_{i(k)}, X_{j(k)}, \theta_{j(k)}) \\
&+ \nabla_{x} \phi(X_{j(k)}, \theta_{j(k)}) \cdot \nabla_{x_2} V(X_{i(k)}, \theta_{i(k)}, X_{j(k)}, \theta_{j(k)}) \Big]
\end{align*}

Par symétrie de la loi de Hooke ($\nabla_{x_2} V(x_1, \theta_1, x_2, \theta_2) = \nabla_{x_1} V(x_2, \theta_2, x_1, \theta_1)$) la somme correspond au potentiel et la fonction test évalués contre la mesure empirique. On obtient alors :
$$
-\mu_X \frac{K}{N} \iint \nabla_{x_1} \phi(x_1, \theta_1) \cdot \nabla_{x_1} V(x_1, \theta_1, x_2, \theta_2) g^K(x_1, \theta_1, x_2, \theta_2) \, dx_1 d\theta_1 dx_2 d\theta_2
$$

Pour les interactions fibres-fibres et fibres-cellules la réécriture est la même que pour la rpeuve sur les cellules. Pour la réorientation angulaire, c'est la même chose que ci-dessus. On obtient au total :
\begin{align*}
\frac{d}{dt} \langle f^N, \phi \rangle &= - \iint \nabla_x \phi \cdot \left( \iint \nabla_x \phi^{FS} h^M \, dz d\alpha + \iint \nabla_x \phi^{FF} f^N \, dx' d\theta' \right) \mu_X f^N \, dx d\theta \\
&\quad - \frac{K}{N} \mu_X \iint \nabla_{x_1} \phi \cdot \nabla_{x_1} V \, g^K \, dx_1 d\theta_1 dx_2 d\theta_2 \\
&\quad - \iint \partial_\theta \phi \left( \iint \partial_\theta \phi^{FS} h^M \, dz d\alpha + \iint \partial_\theta \phi^{FF} f^N \, dx' d\theta' \right) \mu_\theta f^N \, dx d\theta \\
&\quad - \frac{K}{N} \mu_\theta \iint \partial_{\theta_1} \phi \left( \partial_{\theta_1} V + \partial_{\theta_1} W^{al} \right) g^K \, dx_1 d\theta_1 dx_2 d\theta_2
\end{align*}

\subsection{Passage à la limite de champ moyen ($M, N \to \infty$)}
On admet à nouveau la convergence faible des mesures empiriques ($h^M \rightharpoonup h$, $f^N \rightharpoonup f$, $g^K \rightharpoonup g$) en passant à la limite et en transférant les opérateurs par IPP on obtient l'EDP de la forme :
\begin{equation}
\partial_t f + \nabla_x \cdot J_1[g,h] + \partial_\theta J_2[g,h] = 0
\end{equation}

Avec les flux $J_1$ (spatial) et $J_2$ (angulaire) :
\begin{align*}
J_1[g,h](x,\theta) = -\mu_X \Bigg[ &f(x, \theta) \iint \nabla_x \phi^{FS}(x, \theta, z) h(z, \alpha) \, dz d\alpha + f(x, \theta) \iint \nabla_x \phi^{FF}(x, \theta, x', \theta') f(x', \theta') \, dx' d\theta' \\
&+ \xi \iint \nabla_x V(x, \theta, x', \theta') g(x, \theta, x', \theta') \, dx' d\theta' \Bigg]
\end{align*}
\begin{align*}
J_2[g,h](x,\theta) = -\mu_\theta \Bigg[ &f(x, \theta) \iint \partial_\theta \phi^{FS}(x, \theta, z) h(z, \alpha) \, dz d\alpha + f(x, \theta) \iint \partial_\theta \phi^{FF}(x, \theta, x', \theta') f(x', \theta') \, dx' d\theta' \\
&+ \xi \iint \left( \partial_\theta V(x, \theta, x', \theta') + \partial_\theta W^{al}(\theta, \theta') \right) g(x, \theta, x', \theta') \, dx' d\theta' \Bigg]
\end{align*}
